...
const STORAGE_KEY = "properties-data";
    ...
    () => [
      {
        ...
        tasks: [
          ...
          {
            id: "1.2",
            name: "Review client specific notes and warnings",
            notes: "Check sheet for client-specific requirements",
          },
          {
            id: "1.3",
            name: "Understand markup restriction during listing connection",
            notes: "Do NOT update markups when connecting - mainly for BDC",
          },
          {
            id: "1.4",
            name: "Prevent accidental bookings setup",
            notes: "Snooze property, block dates, or set price to 9999",
          },
        ],
      },
      {
        ...
        tasks: [
          {
            id: "2.1",
            name: "Send/accept co-host invitation",
            notes: "Add client code to listing nickname. A008/A006/A044 do not add co-host",
          },
          {
            id: "2.2",
            name: "Add description and amenities",
            notes: "Use bare minimum if client doesn't provide",
          },
          {
            id: "2.3",
            name: "Add photos with descriptions",
            notes: "Ensure bedroom/bed count matches",
          },
          {
            id: "2.4",
            name: "Add fees and policies in Hospitable",
            notes: "Add on Airbnb first, then Hospitable. Must add log image",
          },
  ...
  const updateTask = (propertyId, categoryId, taskId, completed, note = "", userName = "") => {
    setProperties((prevProperties) => {
      const updatedProperties = prevProperties.map((prop) => {
        ...
        const updatedChecklist = prop.checklist.map((cat) => {
          ...
          return {
            ...
            tasks: cat.tasks.map((task) => {
              ...
              const update = note
                ? { note, userName: userName || "Unknown User", timestamp: new Date().toISOString() }
                : null;
        ...
        const completedTasks = updatedChecklist.reduce((sum, cat) => sum + cat.tasks.filter((t) => t.completed).length, 0);
        ...
        return { ...prop, checklist: updatedChecklist, progress, status, lastUpdated: new Date().toISOString() };
      });
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50">
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <StatCard title="Total Properties" value={properties.length} icon={<CheckSquare />} color="blue" />
          <StatCard title="In Progress" value={properties.filter((p) => p.status === "In Progress").length} icon={<Clock />} color="yellow" />
          <StatCard title="Completed" value={properties.filter((p) => p.status === "Completed").length} icon={<CheckCircle2 />} color="green" />
          <StatCard title="Not Started" value={properties.filter((p) => p.status === "Not Started").length} icon={<AlertCircle />} color="gray" />
        </div>
        ...
        {filteredProperties.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm p-12 text-center">
            ...
            <p className="text-gray-500 mb-6">
              {properties.length === 0 ? "Get started by adding your first property" : "Try adjusting your search or filters"}
            </p>
            {properties.length === 0 && (
              <button
                onClick={() => setShowAddModal(true)}
      ...
      {showAddModal && <AddPropertyModal onClose={() => setShowAddModal(false)} onAdd={addProperty} />}
    </div>
  );
};
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow cursor-pointer overflow-hidden">
      ...
      <div className="border-t border-gray-100 px-6 py-3 flex justify-between items-center bg-gray-50">
        <button onClick={onClick} className="text-indigo-600 hover:text-indigo-700 font-medium text-sm flex items-center gap-1">
          <Eye size={16} />
          View Details
        </button>
        <button
...
const PropertyDetail = ({ property, onBack, onUpdateTask, expandedTasks, toggleTaskExpanded }) => {
  ...
  const completedTasks = localProperty.checklist.reduce((sum, cat) => sum + cat.tasks.filter((t) => t.completed).length, 0);
    ...
    setLocalProperty((prev) => ({
      ...
      checklist: prev.checklist.map((cat) => {
        ...
        return {
          ...
          tasks: cat.tasks.map((task) => {
            if (task.id !== taskId) return task;
            const update = note ? { note, userName: name, timestamp: new Date().toISOString() } : null;
            return {
              ...task,
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 pb-12">
      <div className="bg-white shadow-sm sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-6 py-4">
          <button onClick={onBack} className="text-indigo-600 hover:text-indigo-700 font-medium mb-4 flex items-center gap-2">
            ← Back to Dashboard
          </button>
          ...
          <div className="w-full bg-gray-200 rounded-full h-3 mt-4">
            <div className="bg-indigo-600 h-3 rounded-full transition-all" style={{ width: `${property.progress || 0}%` }} />
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 mt-6">
        <div className="bg-white rounded-xl shadow-sm p-4 mb-4">
          <label className="block text-sm font-medium text-gray-700 mb-2">Your Name (for task tracking)</label>
          <input
            type="text"
            value={userName}
              ...
              <div key={category.id} className="bg-white rounded-xl shadow-sm overflow-hidden">
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    <div className="w-32">
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div className="bg-indigo-600 h-2 rounded-full transition-all" style={{ width: `${categoryProgress}%` }} />
                      </div>
                    </div>
                    <span className="text-sm font-semibold text-gray-700 w-12 text-right">{categoryProgress}%</span>
                  </div>
                </button>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {task.completed && (
                                <div className="mt-2 text-xs text-gray-600 flex items-center gap-2">
                                  <User size={14} />
                                  <span>Completed by {task.completedBy} on {new Date(task.completedAt).toLocaleDateString()}</span>
                                </div>
                              )}
...
const AddPropertyModal = ({ onClose, onAdd }) => {
  ...
  const handleCreateClick = () => {
    if (formData.name.trim()) onAdd(formData);
    else alert("Property name is required");
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Property ID / Code</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Channel Manager</label>
            <select
